...
class Variant:
    ...
    def __init__(self, logger, variant=None):
        ...
        # TO-DO: MOVE THIS TO A CONFIGURABLE MAPPING FOR GENE RENAMING
    ...
    def extract_alternate_consequences(self, parent_row, row_list, genes_reported):
        ...
        if hasattr(self, "consequences") and len(self.consequences) > 0:
            self.logger.debug("VAR:extract_alternate_consequences:Now iterating through alternate_consequences")
            ...
            for item in self.consequences:
                ...
                if var.gene_name != parent_row.tbprofiler_gene_name:
                    ...
                    if len(var.annotation) == 0:
                        self.logger.debug("VAR:extract_alternate_consequences:There are no annotations for this alternate consequence; creating a row with the gene associated drugs")
                        ...
                        self.logger.debug("VAR:extract_alternate_consequences:There are {} annotation(s) for this alternate consequence; creating a row for each annotation".format(len(var.annotation)))
    ...
    def extract_annotations(self):
        ...
        if hasattr(self, "annotation") and len(self.annotation) > 0:
            self.logger.debug("VAR:extract_annotations:Starting to turn each annotation into a Row")
            ...
            for item in self.annotation:      
                ...
                if annotation.antimicrobial not in self.annotation_dictionary.keys():
                    self.logger.debug("VAR:extract_annotations:This is the first time this drug ({}) has been seen; adding it to the annotation dictionary".format(annotation.antimicrobial))
                    ...
                    if annotation.antimicrobial in gene_associated_drug_list:
                        ...
                        self.logger.debug("VAR:extract_annotations:The gene associated drug list: {}".format(gene_associated_drug_list))
                ...
                # otherwise, save only the annotation with the more severe WHO confidence (higher value)
                    ...
                    self.logger.debug("VAR:extract_annotations:A more severe WHO confidence annotation was found for this drug ({}); replacing the old annotation with the new one".format(annotation.antimicrobial))
                    self.annotation_dictionary[annotation.antimicrobial] = Row(self.logger, self, annotation.confidence, annotation.antimicrobial, source=annotation.source, tbdb_comment=annotation.tbdb_comment)

            self.logger.debug("VAR:extract_annotations:The annotation dictionary has been expanded; it now has a length of {}".format(len(self.annotation_dictionary)))
            ...
            for drug in gene_associated_drug_list:  
                self.logger.debug("VAR:extract_annotations:The drug ({}) was not found in the annotation dictionary; adding it with a WHO confidence of 'No WHO annotation'".format(drug))
            ...
            if self.gene_name in globals_.GENE_TO_ANTIMICROBIAL_DRUG_NAME.keys():
                for antimicrobial in globals_.GENE_TO_ANTIMICROBIAL_DRUG_NAME[self.gene_name]:
                    if antimicrobial not in self.annotation_dictionary.keys():
                        self.logger.debug("VAR:extract_annotations:The drug ({}) was not found in the gene associated drug list; adding it with a WHO confidence of 'No WHO annotation'".format(antimicrobial))
            ...
            self.logger.debug("VAR:extract_annotations:The annotation dictionary has all gene associated drugs included; it now has a length of {}".format(len(self.annotation_dictionary)))

        else:
            # possibilities 1b and 2: the annotation field has no content or the field does not exist
            self.logger.debug("VAR:extract_annotations:The annotation field has no content or does not exist. Now iterating through gene associated drugs and gene-drug combination dictionary.")
            ...
            self.logger.debug("VAR:extract_annotations:After iterating through gene_associated_drugs, the annotation dictionary has a length of {}".format(len(self.annotation_dictionary)))
